...
const root = buildRouteMap({
    routes: {
        ...
        reconcile: buildCommand({
            loader: async () => (await import("./reconcile")).report,
            parameters: { flags: { fix: bool("repair what can be repaired") } },
            docs: { brief: "report drift between recorded state and the repository" },
        }),
